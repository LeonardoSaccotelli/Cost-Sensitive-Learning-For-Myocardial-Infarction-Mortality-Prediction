...
.PHONY: cleaning
cleaning:
	$(PYTHON_INTERPRETER) myocardial_infarction_mortality/cleaning.py

## Make features (use 'make features' to generate the final processed dataset for modeling)
.PHONY: features
features:
	$(PYTHON_INTERPRETER) myocardial_infarction_mortality/features.py

## Make train (use 'make train' to run the training.py process)
.PHONY: train
train:
	$(PYTHON_INTERPRETER) myocardial_infarction_mortality/modeling/train.py
