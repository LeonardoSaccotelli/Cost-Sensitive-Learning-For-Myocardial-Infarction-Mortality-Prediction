...
Unlike static methods, DES tailors the active classifier subset for each specific query instance based on a local region of competence \cite{CRUZ2018195}. Operating on a base classifier pool generated by a BaggingClassifier of decision trees, we evaluated three distinct DES paradigms. To ensure comparability, competence estimation was performed on a dedicated dynamic selection dataset (DSEL). All models shared a neighborhood size of \texttt{k=8}, an instance hardness threshold of \texttt{IH\_rate=0.3}, and soft voting aggregation. Building on this baseline, we explored three distinct paradigms. First, \textbf{KNORAE} (an oracle-based method) identifies local experts that perfectly classify a query's nearest neighbors, dynamically shrinking the region if no perfect oracle is initially found. Second, \textbf{DESKL} (a probabilistic method) evaluates competence over the entire DSEL via a distance-weighted Gaussian potential function, selecting classifiers based on the Kullback--Leibler divergence of their output profiles. Third, \textbf{METADES} (a meta-learning approach) extracts local meta-features to train a Multinomial Naive Bayes meta-classifier that explicitly predicts base-learner competence. Minor model-specific behaviors, such as dynamic frienemy pruning, followed standard literature defaults.
