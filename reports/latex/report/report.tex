...
We also considered a heterogeneous \texttt{StackingClassifier} (Stacking) combining RF, XGB, and SGD as first-level learners. Their outputs were then combined by a LR meta-estimator. The meta-model was trained using cross-validated predictions of the base learners with \texttt{cv=3}, allowing the second-level learner to exploit complementary predictive patterns captured by the first-level models. As with the Voting ensemble, the constituent learners were independently re-instantiated and optimized within the pipeline.
\end{itemize}

\subsubsection{Dynamic Ensemble Selection} Dynamic Ensemble Selection (DES) methods select, for each query sample, the subset of classifiers that is estimated to be the most competent in its local context, rather than relying on a fixed ensemble for all instances. According to the taxonomy proposed in \cite{CRUZ2018195}, a DES system typically involves three key elements: the definition of a region of competence around the query sample, the estimation of the local competence of the classifiers in the pool, and the dynamic selection of the ensemble used for the final prediction. 

In our setting, the DES models operated on a classifier pool generated by a \texttt{BaggingClassifier} with \texttt{DecisionTreeClassifier} as the base estimator, while competence estimation was performed on a dedicated dynamic selection dataset (DSEL). We considered three representative DES approaches spanning different competence estimation paradigms identified in the literature: an oracle-based method (\texttt{KNORAE}), a probabilistic method (\texttt{DESKL}), and a meta-learning-based method (\texttt{METADES}). All DES models were configured with a neighborhood size of \texttt{k=8}, using a k-nearest neighbors search with Minkowski distance for competence estimation. In addition, all methods used \textit{soft voting} to aggregate the predictions of the selected classifiers. To improve robustness in borderline regions, we adopted an instance hardness threshold of \texttt{IH\_rate=0.3}. These shared settings ensured a comparable experimental configuration across the considered DES techniques, while allowing each method to preserve its own distinct competence estimation mechanism. 

\begin{itemize} 
\item \textbf{KNORA-E.} \texttt{KNORAE} (\textit{K-Nearest Oracles Eliminate}) is an oracle-based DES method relying on a k-nearest neighbors region of competence. For a given query sample, the method searches for classifiers that correctly classify all samples in the local neighborhood. If no classifier satisfies this condition, the neighborhood is progressively reduced until at least one competent classifier is identified. The final decision is then obtained by aggregating the predictions of the selected local experts. In our implementation, \texttt{KNORAE} was used with \texttt{DFP=True} to enable dynamic frienemy pruning, and \texttt{knne=True}, ensuring the region of competence was built through the K-Nearest Neighbor Equality strategy. This configuration favors classifiers that behave as local oracles in the vicinity of the query sample. 

\item \textbf{DES-KL.} \texttt{DESKL} is a probabilistic DES method based on a potential function model. Unlike standard neighborhood-based techniques, it estimates competence using the whole DSEL, while weighting each sample according to its Euclidean distance from the query through a Gaussian potential function. The source of competence is derived from an information-theoretic perspective, namely the Kullback--Leibler divergence between the uniform distribution and the vector of class supports produced by each classifier. Classifiers with positive competence are selected to compose the dynamic ensemble. In our implementation, \texttt{DESKL} was used in \texttt{mode="selection"}, so that only the classifiers deemed locally competent contributed to the final prediction. This makes \texttt{DESKL} a probabilistic alternative to strictly neighborhood-based DES methods.

\item \textbf{META-DES.} \texttt{METADES} reformulates dynamic selection as a meta-learning problem. Instead of relying on a single competence criterion, it extracts a set of meta-features that characterize the behavior of each base classifier with respect to the query sample. These meta-features are then provided to a meta-classifier, which predicts whether a given classifier is competent or incompetent for that instance. By combining multiple sources of information into a learned competence model, \texttt{METADES} provides a more flexible and expressive mechanism for dynamic ensemble selection. In our implementation, \texttt{METADES} was configured with \texttt{Kp=8} for the output-profile-based competence estimation, together with \texttt{DFP=True}, \texttt{knne=True}, and \texttt{mode="selection"}. This configuration preserves the original meta-learning rationale of the method while enforcing a selection-based decision process. 
\end{itemize}

\begin{table*}[t]
\centering
\caption{Predictive models and their respective hyperparameter search spaces.}
\label{tab:model-search-space}
\begin{threeparttable}
...
\begin{figure*}[h]
    ...
    \caption{Pairwise p-value heatmap of the corrected resampled t-test on Average Cost across all evaluated configurations.}
