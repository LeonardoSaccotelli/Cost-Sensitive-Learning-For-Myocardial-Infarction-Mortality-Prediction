...
\begin{frame}
\frametitle{Data Cleaning \& Preprocessing (1/2)}
...
\begin{frame}
\frametitle{Data Cleaning \& Preprocessing (2/2)}   
...
\begin{frame}
\frametitle{Predictive Models: Dynamic Ensemble Selection (1/4)}

	\begin{block}{The Dynamic Paradigm \cite{CRUZ2018195}}
		Unlike static models that rely on a fixed ensemble for all instances, DES selects the most competent subset of classifiers for \textit{each specific query sample} based on its local context.
	\end{block}
	
	\begin{block}{The 3-Phase Architecture}
		\begin{enumerate}
			\item \textbf{Generation:} A diverse pool of classifiers is built.
			\item \textbf{Selection (Competence Estimation):} For a new query, a local "region of competence" is defined using a dedicated dataset (\textbf{DSEL}). The system evaluates and selects the local experts.
...
\begin{frame}
\frametitle{Predictive Models: Dynamic Ensemble Selection (2/4)}

	\begin{center}		
		\vspace{0.3em}			\includegraphics[width=0.95\textwidth,height=0.4\textheight,keepaspectratio]{figures/des1.png}
...
\begin{frame}
\frametitle{Predictive Models: Dynamic Ensemble Selection (3/4)}
...
\begin{frame}
\frametitle{Predictive Models: Dynamic Ensemble Selection (4/4)}
...
\begin{frame}
\frametitle{Imbalance-Aware: Learning-Level Strategies (1/3)}
...
\begin{frame}
\frametitle{Imbalance-Aware: Learning-Level Strategies (2/3)}
...
\begin{frame}
\frametitle{Imbalance-Aware: Learning-Level Strategies (3/3)}
...
\begin{frame}
\frametitle{Learning Pipeline (1/2): Baselines \& Static Ensembles}
...
\begin{frame}
\frametitle{Learning Pipeline (2/2): Dynamic Ensembles}
...
\frametitle{Conclusions \& Future Directions}
	...
	\textbf{Key Findings}
	\begin{itemize}
		\item \textbf{Core Necessity:} Addressing class imbalance via cost-sensitive learning is crucial.
        \item \textbf{Optimal Setup:} Cost-sensitive learning combined with a standard decision threshold yielded the most reliable performance.
        \item \textbf{Top Architectures:} Static ensembles (\texttt{StackingClassifier}, \texttt{XGBClassifier}) robustly outperformed the best dynamic method (\texttt{DES-KL}).
    \end{itemize}
	...
	\begin{itemize}
		\item \textbf{Data Validation:} Validate the framework on modern, large-scale clinical datasets.
        \item \textbf{Algorithmic Exploration:} Investigate alternative DES pool-generation and threshold calibration techniques to further optimize the FP vs. FN trade-off.
    \end{itemize}
